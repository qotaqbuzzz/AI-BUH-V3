...
} as const satisfies Record<DocumentType, DocumentMeta>;
...
const filename = (type: DocumentType, suffix: string): string =>
  `${type}_${suffix}_${today()}.html`;

// ── HTML base layout ───────────────────────────────────────────────────────

function htmlDoc(title: string, body: string): HtmlContent {
  return `<!DOCTYPE html>
<html lang="ru">
<head>
  <meta charset="UTF-8">
...
export class DocumentGeneratorService {
  ...
  async generateActSverki(params: ActSverkiParams): Promise<GeneratedDocument> {
    ...
    type LedgerRow = { date: string; doc: string; debit: number; credit: number; purpose: string };
  ...
  async generateDebtCertificate(params: DebtCertParams): Promise<GeneratedDocument> {
  ...
  async generateCreditorsReport(params: CreditorsReportParams): Promise<GeneratedDocument> {
    const asOf = params.date ?? today();
  ...
  async generateObligationNotice(params: ObligationNoticeParams): Promise<GeneratedDocument> {
    const asOf = params.date ?? today();
    const [balance, orgs] = await Promise.all([
      this.reports.getContractorBalance(params.contractorGuid, asOf),
      this.catalog.getOrganizations(),
