...
export class InvestigationService {
  ...
  async investigatePayment(
    ...
    const doc = await this.client
      .getCollection<PaymentDoc>("Document_ПлатежноеПоручениеИсходящее", {
  ...
  async findDuplicatePairs(
    ...
    const kaGuids = [...new Set(docs.map(d => d.Контрагент_Key).filter(Boolean) as string[])];
    ...
    for (let i = 0; i < sorted.length; i++) {
      ...
      for (let j = i + 1; j < sorted.length; j++) {
        ...
        if (a.СуммаДокумента !== b.СуммаДокумента) continue;
        if (a.Контрагент_Key !== b.Контрагент_Key) continue;
        if (a.Ref_Key === b.Ref_Key) continue;
        ...
        if (vedA || vedB) {
          const emp = await this.resolveEmployee(
            (vedA ? [{ Ведомость_Key: vedA }] : vedB ? [{ Ведомость_Key: vedB }] : [])
          );
          if (emp.found) {
        ...
        const riskLevel: "high" | "medium" =
          (samePurpose && sameAccount) || sameEmployee ? "high" : "medium";
  ...
  private async resolveOrgAccount(
    guid?: string,
  ): Promise<PaymentInvestigation["orgAccount"]> {
    const empty = { guid: guid ?? "—", accountNumber: "—", description: "—", bank: "—" };
    if (!guid || guid === "00000000-0000-0000-0000-000000000000") return empty;

    const rows = await this.client
      .getCollection<any>("Catalog_БанковскиеСчета", {
    ...
    if (rows[0].Банк_Key && rows[0].Банк_Key !== "00000000-0000-0000-0000-000000000000") {
      const b = await this.client
        .getCollection<any>("Catalog_Банки", {
  ...
  private async resolveContractor(guid?: string): Promise<string | undefined> {
    if (!guid || guid === "00000000-0000-0000-0000-000000000000") return undefined;
    const rows = await this.client
      .getCollection<any>("Catalog_Контрагенты", {
        filter: `Ref_Key eq guid'${guid}'`,
        select: "Description",
        top: 1,
      })
  ...
  private async resolveEmployee(
    zpRows: ZPRow[],
  ): Promise<PaymentInvestigation["employee"]> {
    ...
    const veds = await this.client
      .getCollection<any>("Document_ЗарплатаКВыплатеОрганизаций", {
        filter: `Ref_Key eq guid'${vedKey}'`,
        top: 1,
      })
      .catch(() => []);
    if (!veds.length) return { found: false };
    ...
    if (fizKey && fizKey !== "00000000-0000-0000-0000-000000000000") {
      const fiz = await this.client
        .getCollection<any>("Catalog_ФизическиеЛица", {
  ...
  private buildVerdict(
    doc:      PaymentDoc,
    employee: PaymentInvestigation["employee"],
  ): string {
    const parts: string[] = [];
    if (!doc.Posted) parts.push("⚠ Документ НЕ проведён — проводки не созданы");
    if (employee.found) {
  ...
  private calcRisk(
    doc:      PaymentDoc,
    employee: PaymentInvestigation["employee"],
  ): PaymentInvestigation["riskLevel"] {
    if (employee.found && !employee.vedPosted) return "high";
    if (!doc.Posted) return "medium";
    return "low";
  }

  private notFound(number: string, date: string): PaymentInvestigation {
    return {
  ...
  private async batchNames(
    ...
    for (let i = 0; i < guids.length; i += BATCH) {
      ...
      const rows = await this.client
        .getCollection<any>(entity, { filter: f, select: `Ref_Key,${field}`, top: BATCH })
        .catch(() => []);
      for (const r of rows) map.set(r.Ref_Key, String(r[field] ?? ""));
    }
    return map;
  }
}
