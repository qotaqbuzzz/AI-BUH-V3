...
import type { AccountingBalanceAndTurnover } from "@aibos/onec-client";
...
export interface AnomalyReport {
  manualEntries: { guid: string; date: string; number: string; amount: number; comment: string }[];
  roundAmounts: { guid: string; date: string; number: string; amount: number; docType: string }[];
  unposted: { docType: string; count: number }[];
  summary: { manual: number; round: number; unposted: number };
}
...
export class ReportsService {
  ...
  async getOSV(dateFrom: string, dateTo: string, organizationGuid?: string): Promise<{
    rows: OSVRow[];
    totals: { openDr: number; openCr: number; turnDr: number; turnCr: number; closeDr: number; closeCr: number };
  }> {
    const accounts = await this.client.getCollection<{ Ref_Key: string; Code: string; Description: string }>(
      "ChartOfAccounts_Типовой",
      { select: "Ref_Key,Code,Description", top: 1000 },
    );
    ...
    const rows = await this.client.getBalanceAndTurnovers<AccountingBalanceAndTurnover>(
    ...
    const grouped = new Map<string, { openDr: number; openCr: number; turnDr: number; turnCr: number; closeDr: number; closeCr: number }>();
    ...
    for (const [accountGuid, v] of grouped) {
      if (v.openDr === 0 && v.openCr === 0 && v.turnDr === 0 && v.turnCr === 0 && v.closeDr === 0 && v.closeCr === 0) continue;
  ...
  private async resolveNames(guids: string[]): Promise<Map<string, string>> {
    ...
    for (let i = 0; i < unique.length; i += BATCH) {
      ...
      const [contractors, employees] = await Promise.all([
        this.client.getCollection<{ Ref_Key: string; Description: string }>(
          "Catalog_Контрагенты",
          { filter, select: "Ref_Key,Description", top: BATCH },
      ...
      for (const c of [...contractors, ...employees]) nameMap.set(c.Ref_Key, c.Description);
    }
    return nameMap;
  }

  private async getSettlementRows(accountCodes: string[], organizationGuid?: string, date?: string): Promise<{
    rows: SettlementRow[];
    byAccount: Record<string, number>;
  }> {
    const accountRows = await this.client.getCollection<{ Ref_Key: string; Code: string }>(
      "ChartOfAccounts_Типовой",
      {
    ...
    if (!accountRows.length) return { rows: [], byAccount: {} };
    ...
    for (const acc of accountRows) {
      ...
      const balRows = await this.client.getRegisterBalance<{
    ...
    for (const row of allRows) row.contractorName = nameMap.get(row.contractorGuid) ?? row.contractorGuid.slice(0, 8);

    return { rows: allRows, byAccount };
  }

  async getAllDebtors(organizationGuid?: string, date?: string): Promise<{
    rows: SettlementRow[];
    total: number;
    byAccount: Record<string, number>;
  }> {
  ...
  async getAllCreditors(organizationGuid?: string, date?: string): Promise<{
    rows: SettlementRow[];
    total: number;
    byAccount: Record<string, number>;
  }> {
  ...
  async getIncomingPayments(dateFrom: string, dateTo: string, contractorGuid?: string, organizationGuid?: string): Promise<{
    rows: {
      date: string;
      number: string;
      contractor: string;
    ...
    const docs = await this.client.getCollection<{
      Ref_Key: string; Date: string; Number: string;
      СуммаДокумента?: number; ВалютаДокумента_Key?: string;
      Контрагент_Key?: string; НазначениеПлатежа?: string; Posted?: boolean;
    ...
    const contractorGuids = [...new Set(docs.map(d => d.Контрагент_Key).filter(Boolean) as string[])];
    const currencyGuids   = [...new Set(docs.map(d => d.ВалютаДокумента_Key).filter(Boolean) as string[])];

    const [contractorNames, currencyNames] = await Promise.all([
      this.resolveNames(contractorGuids),
      (async () => {
        const map = new Map<string, string>();
        if (!currencyGuids.length) return map;
        const BATCH = 10;
        for (let i = 0; i < currencyGuids.length; i += BATCH) {
          ...
          const rows = await this.client.getCollection<{ Ref_Key: string; Description: string }>(
            "Catalog_Валюты", { filter: batch.map(g => `Ref_Key eq guid'${g}'`).join(" or "), select: "Ref_Key,Description", top: BATCH },
          ).catch(() => [] as { Ref_Key: string; Description: string }[]);
          for (const r of rows) map.set(r.Ref_Key, r.Description);
        }
        return map;
      })(),
    ]);
  ...
  async getOutgoingPayments(dateFrom: string, dateTo: string, contractorGuid?: string, organizationGuid?: string): Promise<{
    rows: {
      date: string; number: string; contractor: string; currency: string;
      amount: number; purpose: string; posted: boolean; docGuid: string;
    }[];
    totalByCurrency: Record<string, number>;
  }> {
    const filters = [
  ...
  async getPurchasesReport(dateFrom: string, dateTo: string, contractorGuid?: string, organizationGuid?: string): Promise<{
    rows: {
      date: string; docNumber: string; contractor: string; contract: string;
      currency: string; lineType: string; item: string; qty: number; uom: string;
      price: number; amount: number; vatPct: string; vatAmount: number; total: number; docGuid: string;
    }[];
    totals: { amount: number; vatAmount: number; total: number };
  }> {
    const filters = [
    ...
    const docs = await this.client.getCollection<{
      Ref_Key: string; Date: string; Number: string;
      Контрагент_Key?: string; ДоговорКонтрагента_Key?: string;
    ...
    if (!docs.length) return { rows: [], totals: { amount: 0, vatAmount: 0, total: 0 } };

    const docGuids = docs.map(d => d.Ref_Key);
    const docMap = new Map(docs.map(d => [d.Ref_Key, d]));

    // Fetch tabular parts
    const fetchRows = async (suffix: string) => {
      const BATCH = 20;
      const all: { Ref_Key: string; Номенклатура_Key?: string; Количество?: number; ЕдиницаИзмерения_Key?: string; Цена?: number; Сумма?: number; СуммаНДС?: number }[] = [];
      for (let i = 0; i < docGuids.length; i += BATCH) {
        const batch = docGuids.slice(i, i + BATCH);
        const r = await this.client.getCollection<typeof all[0]>(`Document_ПоступлениеТоваровУслуг_${suffix}`, {
    ...
    const contrGuids = [...new Set(docs.map(d => d.Контрагент_Key).filter(Boolean) as string[])];
    const contractGuids = [...new Set(docs.map(d => d.ДоговорКонтрагента_Key).filter(Boolean) as string[])];
    const currGuids = [...new Set(docs.map(d => d.ВалютаДокумента_Key).filter(Boolean) as string[])];
    const nomGuids = [...new Set(allLineRows.map(r => r.Номенклатура_Key).filter(Boolean) as string[])];

    const loadNames = async (guids: string[], entity: string) => {
      const map = new Map<string, string>();
      const BATCH = 10;
      for (let i = 0; i < guids.length; i += BATCH) {
        const batch = guids.slice(i, i + BATCH);
        const rows = await this.client.getCollection<{ Ref_Key: string; Description: string }>(
          entity, { filter: batch.map(g => `Ref_Key eq guid'${g}'`).join(" or "), select: "Ref_Key,Description", top: BATCH },
        ).catch(() => [] as { Ref_Key: string; Description: string }[]);
        for (const r of rows) map.set(r.Ref_Key, r.Description);
      }
      return map;
    ...
    const rows: ReturnType<typeof this.getPurchasesReport> extends Promise<{ rows: infer R }> ? R : never = [];
    const addRows = (lineRows: typeof allLineRows, lineType: string) => {
      for (const row of lineRows) {
        ...
        (rows as typeof rows).push({
          date: doc.Date?.slice(0, 10) ?? "",
          docNumber: (doc as { Number?: string }).Number ?? "",
    ...
    (rows as typeof rows).sort((a, b) => a.date.localeCompare(b.date));

    const totals = {
      amount: r2((rows as typeof rows).reduce((s, r) => s + r.amount, 0)),
      vatAmount: r2((rows as typeof rows).reduce((s, r) => s + r.vatAmount, 0)),
      total: r2((rows as typeof rows).reduce((s, r) => s + r.total, 0)),
    };

    return { rows: rows as typeof rows, totals };
  }

  async getContractorBalance(contractorGuid: string, date?: string): Promise<{
    contractorName: string;
    rows: { accountCode: string; accountName: string; balanceDr: number; balanceCr: number; net: number }[];
    totals: { balanceDr: number; balanceCr: number; net: number };
  }> {
    const [contrRows, accounts, balRows] = await Promise.all([
      this.client.getCollection<{ Ref_Key: string; Description: string }>(
        "Catalog_Контрагенты",
        { filter: `Ref_Key eq guid'${contractorGuid}'`, select: "Ref_Key,Description", top: 1 },
      ).catch(() => [] as { Ref_Key: string; Description: string }[]),
      this.client.getCollection<{ Ref_Key: string; Code: string; Description: string }>(
        "ChartOfAccounts_Типовой", { select: "Ref_Key,Code,Description", top: 1000 },
      ),
      this.client.getRegisterBalance<{
        Account_Key?: string; ExtDimension1?: string; ExtDimension2?: string;
        СуммаBalanceDr?: number; СуммаBalanceCr?: number;
      }>("AccountingRegister_Типовой", {
    ...
    const contrBalRows = balRows.filter(r =>
      r.ExtDimension1 === contractorGuid || r.ExtDimension2 === contractorGuid,
    );

    const byAcc = new Map<string, { dr: number; cr: number }>();
  ...
  async detectAnomalies(dateFrom: string, dateTo: string, organizationGuid?: string): Promise<AnomalyReport> {
    const baseFilters = [
      `Date ge datetime'${dateFrom}T00:00:00'`,
      `Date le datetime'${dateTo}T23:59:59'`,
    ];
    if (organizationGuid) baseFilters.push(`Организация_Key eq guid'${organizationGuid}'`);
    const filterStr = baseFilters.join(" and ");

    // 1. Manual ОперацияБух entries
    const manualRaw = await this.client.getCollection<{
      Ref_Key: string; Date: string; Number: string;
      СуммаДокумента?: number; Комментарий?: string;
    ...
    const roundAmounts: AnomalyReport["roundAmounts"] = [];
    for (const dt of ["РеализацияТоваровУслуг", "ПоступлениеТоваровУслуг", "ОперацияБух"]) {
      const docs = await this.client.getCollection<{
        Ref_Key: string; Date: string; Number: string; СуммаДокумента?: number;
      }>(`Document_${dt}`, {
        filter: filterStr,
    ...
    if (organizationGuid) unpostedFilters.push(`Организация_Key eq guid'${organizationGuid}'`);
    const unpostedFilter = unpostedFilters.join(" and ");

    const unposted: AnomalyReport["unposted"] = [];
    for (const dt of ["РеализацияТоваровУслуг", "ПоступлениеТоваровУслуг", "ПлатежноеПоручениеИсходящее", "ПлатежноеПоручениеВходящее"]) {
      ...
      if (docs.length > 0) unposted.push({ docType: dt, count: docs.length });
    }
  ...
  async getDetailedCreditors(organizationGuid?: string, date?: string): Promise<{
    ...
    const { rows: baseRows, byAccount } = await this.getSettlementRows(
      ["3310", "3350", "3387", "3390"],
      organizationGuid,
      date,
    );
    ...
    if (!creditorRows.length) return { rows: [], total: 0, byAccount, asOfDate: asOf };

    const BATCH = 8;
    const result: CreditorDetailRow[] = [];
    ...
    for (let i = 0; i < creditorRows.length; i += BATCH) {
      ...
      await Promise.all(batch.map(async (cr) => {
        ...
        const obligationType: CreditorDetailRow["obligationType"] =
          cr.accountCode === "3350" ? "payroll"
          : cr.accountCode === "3387" ? "advance_received"
          : "supplier_debt";

        let firstDocDate: string | null = null;
        let firstDocNumber: string | null = null;
        let lastDocDate: string | null = null;
        const contractSet = new Set<string>();
        let totalPaid = 0;
        let lastPaymentDate: string | null = null;
        let lastPaymentPurpose: string | null = null;
        ...
        if (cr.contractorGuid && obligationType !== "payroll") {
          ...
          const [docs, payments] = await Promise.all([
            this.client.getCollection<{
              Date: string; Number: string; ДоговорКонтрагента_Key?: string;
            }>("Document_ПоступлениеТоваровУслуг", {
              filter: docFilter,
            ...
            this.client.getCollection<{
              Date: string; СуммаДокумента?: number; НазначениеПлатежа?: string;
              ВалютаДокумента_Key?: string;
            }>("Document_ПлатежноеПоручениеИсходящее", {
              filter: [
                "Posted eq true", "DeletionMark eq false",
          ...
          if (docs.length) {
            ...
            for (const d of docs) {
              if (d.ДоговорКонтрагента_Key) contractSet.add(d.ДоговорКонтрагента_Key);
            }
          }
        ...
        if (contractGuids.length) {
          const contractDocs = await this.client.getCollection<{ Ref_Key: string; Description: string }>(
          ...
          for (const c of contractDocs) contracts.push(c.Description ?? c.Ref_Key.slice(0, 8));
        }

        // Age category based on first doc date
        let ageCategory: CreditorDetailRow["ageCategory"] = "unknown";
        if (firstDocDate) {
          const ageMs = new Date(asOf).getTime() - new Date(firstDocDate).getTime();
          const ageYears = ageMs / (1000 * 60 * 60 * 24 * 365);
          if (ageYears < 1) ageCategory = "current";
          else if (ageYears < 2) ageCategory = "1y";
          else if (ageYears < 3) ageCategory = "2y";
          else ageCategory = "3y+";
        } else if (lastDocDate) {
  ...
  async getDetailedAdvancesReceived(organizationGuid?: string, date?: string): Promise<{
    rows: CreditorDetailRow[];
    total: number;
    byAccount: Record<string, number>;
    asOfDate: string;
    ...
    if (!advanceRows.length) return { rows: [], total: 0, byAccount, asOfDate: asOf };
      ...
      await Promise.all(batch.map(async (cr) => {
        ...
        if (cr.contractorGuid) {
          ...
          const [inDocs, salesDocs] = await Promise.all([
            this.client.getCollection<{
              Date: string; Number: string; ДоговорКонтрагента_Key?: string; СуммаДокумента?: number;
            }>("Document_ПлатежноеПоручениеВходящее", {
              filter: baseFilter,
  ...
  async getFullLiabilitiesReport(organizationGuid?: string, date?: string): Promise<{
    asOfDate: string;
    grandTotal: number;
    sections: {
      sectionName: string;
    ...
    const liabilityAccounts = osvRows.rows.filter(r =>
      (r.closingCr > 0) &&
  ...
  async getAdvanceSettlementStatus(
    contractorGuid: string,
    ...
    summary: { totalReceived: number; totalOffset: number; remaining: number; fulfillmentPct: number };
  }> {
    const ADVANCE_ACCOUNTS = ["3510", "3387"];

    // Resolve account GUIDs and names
    const accountRows = await this.client.getCollection<{ Ref_Key: string; Code: string; Description: string }>(
      "ChartOfAccounts_Типовой",
      {
    ...
    const contrRows = await this.client.getCollection<{ Ref_Key: string; Description: string }>(
      "Catalog_Контрагенты",
      { filter: `Ref_Key eq guid'${contractorGuid}'`, select: "Ref_Key,Description", top: 1 },
    ).catch(() => [] as { Ref_Key: string; Description: string }[]);
    const contractorName = contrRows[0]?.Description ?? contractorGuid;
    ...
    for (const acc of accountRows) {
      ...
      const rows = await this.client.getBalanceAndTurnovers<{
        Account_Key?: string;
        ExtDimension1?: string;
      ...
      const contrRows2 = rows.filter(r =>
        r.ExtDimension1 === contractorGuid || r.ExtDimension2 === contractorGuid,
      );
  ...
  async getSalesWithLines(
    contractorGuids: string[],
    dateFrom: string,
    dateTo: string,
    organizationGuid?: string,
  ): Promise<{
    rows: {
      date: string; docNumber: string; docGuid: string; contractorName: string;
      totalAmount: number;
      lines: { item: string; qty: number; unit: string; price: number; amount: number; vatAmount: number }[];
    }[];
    totals: { amount: number; vatAmount: number; docs: number; lines: number };
    byNomenclature: { item: string; totalQty: number; totalAmount: number }[];
  }> {
    if (!contractorGuids.length) return { rows: [], totals: { amount: 0, vatAmount: 0, docs: 0, lines: 0 }, byNomenclature: [] };
    ...
    const docs = await this.client.getCollection<{
      Ref_Key: string; Date: string; Number: string;
      Контрагент_Key?: string; СуммаДокумента?: number;
    }>("Document_РеализацияТоваровУслуг", {
      filter: filters.join(" and "),
    ...
    if (!docs.length) return { rows: [], totals: { amount: 0, vatAmount: 0, docs: 0, lines: 0 }, byNomenclature: [] };
    ...
    const BATCH = 20;
    const allLines: {
      Ref_Key: string; Номенклатура_Key?: string; Количество?: number;
      ЕдиницаИзмерения_Key?: string; Цена?: number; Сумма?: number; СуммаНДС?: number;
    }[] = [];

    for (let i = 0; i < docGuids.length; i += BATCH) {
      const batch = docGuids.slice(i, i + BATCH);
      const [товары, услуги] = await Promise.all([
        this.client.getCollection<typeof allLines[0]>("Document_РеализацияТоваровУслуг_Товары", {
          filter: batch.map(g => `Ref_Key eq guid'${g}'`).join(" or "),
          select: "Ref_Key,Номенклатура_Key,Количество,ЕдиницаИзмерения_Key,Цена,Сумма,СуммаНДС",
          top: BATCH * 100,
        }).catch(() => []),
        this.client.getCollection<typeof allLines[0]>("Document_РеализацияТоваровУслуг_Услуги", {
          filter: batch.map(g => `Ref_Key eq guid'${g}'`).join(" or "),
          select: "Ref_Key,Номенклатура_Key,Количество,ЕдиницаИзмерения_Key,Цена,Сумма,СуммаНДС",
          top: BATCH * 100,
        }).catch(() => []),
    ...
    // Resolve names
    const nomGuids  = [...new Set(allLines.map(l => l.Номенклатура_Key).filter(Boolean) as string[])];
    const uomGuids  = [...new Set(allLines.map(l => l.ЕдиницаИзмерения_Key).filter(Boolean) as string[])];

    const loadNames = async (guids: string[], entity: string) => {
      const map = new Map<string, string>();
      for (let i = 0; i < guids.length; i += 10) {
        const rows = await this.client.getCollection<{ Ref_Key: string; Description: string }>(
          entity, { filter: guids.slice(i, i + 10).map(g => `Ref_Key eq guid'${g}'`).join(" or "), select: "Ref_Key,Description", top: 10 },
        ).catch(() => [] as { Ref_Key: string; Description: string }[]);
        for (const r of rows) map.set(r.Ref_Key, r.Description);
      }
      return map;
    ...
    for (const line of allLines) {
      if (!linesByDoc.has(line.Ref_Key)) linesByDoc.set(line.Ref_Key, []);
    ...
    const nomAgg = new Map<string, { totalQty: number; totalAmount: number }>();
    for (const row of rows) {
  ...
  async getGroupBalance(
    ...
    const groupRows = balRows.filter(r =>
      (r.ExtDimension1 && guidSet.has(r.ExtDimension1)) ||
  ...
  async getSalesReport(
    ...
    const docs = await this.client.getCollection<{
      Ref_Key: string; Date: string; Number: string;
      Контрагент_Key?: string; ДоговорКонтрагента_Key?: string; ВалютаДокумента_Key?: string;
    }>("Document_РеализацияТоваровУслуг", {
      filter: filters.join(" and "),
    ...
    type LineRow = { Ref_Key: string; Номенклатура_Key?: string; Количество?: number; ЕдиницаИзмерения_Key?: string; Цена?: number; Сумма?: number; СуммаНДС?: number };
    const fetchLines = async (suffix: string): Promise<LineRow[]> => {
      const BATCH = 20;
      const all: LineRow[] = [];
      for (let i = 0; i < docGuids.length; i += BATCH) {
        const batch = docGuids.slice(i, i + BATCH);
        const r = await this.client.getCollection<LineRow>(`Document_РеализацияТоваровУслуг_${suffix}`, {
          filter: batch.map(g => `Ref_Key eq guid'${g}'`).join(" or "),
          select: "Ref_Key,Номенклатура_Key,Количество,ЕдиницаИзмерения_Key,Цена,Сумма,СуммаНДС",
          top: BATCH * 50,
        }).catch(() => []);
    ...
    type SalesRow = { date: string; docNumber: string; contractor: string; contract: string; currency: string; lineType: string; item: string; qty: number; uom: string; price: number; amount: number; vatPct: string; vatAmount: number; total: number; docGuid: string };
    const rows: SalesRow[] = [];

    const addRows = (lineRows: LineRow[], lineType: string) => {
      for (const row of lineRows) {
        const doc = docMap.get(row.Ref_Key);
        if (!doc) continue;
        const amount = row.Сумма ?? 0;
  ...
  async getCashFlowSummary(
    ...
    type PayDoc = { Date: string; СуммаДокумента?: number };
    ...
    const addToMonth = (docs: PayDoc[], isInflow: boolean) => {
      for (const d of docs) {
        const month = (d.Date ?? "").slice(0, 7);
        if (!month) continue;
        const g = monthMap.get(month) ?? { inflow: 0, outflow: 0 };
        if (isInflow) g.inflow += d.СуммаДокумента ?? 0;
        else g.outflow += d.СуммаДокумента ?? 0;
    ...
    const sum = (docs: PayDoc[]) => r2(docs.reduce((s, d) => s + (d.СуммаДокумента ?? 0), 0));
  ...
  async getFixedAssets(
    ...
    asOfDate: string;
    rows: { guid: string; name: string; initialCost: number; accumulatedDepreciation: number; residualValue: number }[];
    totals: { initialCost: number; accumulatedDepreciation: number; residualValue: number };
  }> {
    const asOf = date ?? new Date().toISOString().slice(0, 10);

    const accRows = await this.client.getCollection<{ Ref_Key: string; Code: string }>(
      "ChartOfAccounts_Типовой",
      { filter: "Code eq '2410' or Code eq '2420'", select: "Ref_Key,Code", top: 10 },
    );
    const acc2410 = accRows.find(a => a.Code === "2410");
    const acc2420 = accRows.find(a => a.Code === "2420");

    type BalRow = { ExtDimension1?: string; СуммаBalanceDr?: number; СуммаBalanceCr?: number };
    const buildFilter = (accGuid: string) => {
    ...
    const [rows2410, rows2420] = await Promise.all([
      acc2410 ? this.client.getRegisterBalance<BalRow>("AccountingRegister_Типовой", {
        filter: buildFilter(acc2410.Ref_Key),
        select: "ExtDimension1,СуммаBalanceDr,СуммаBalanceCr",
        top: 2000,
        ...(date ? { Period: `${date}T23:59:59` } : {}),
      }) : Promise.resolve([] as BalRow[]),
      acc2420 ? this.client.getRegisterBalance<BalRow>("AccountingRegister_Типовой", {
    ...
    for (let i = 0; i < allGuids.length; i += BATCH) {
      ...
      const items = await this.client.getCollection<{ Ref_Key: string; Description: string }>(
        "Catalog_ОсновныеСредства",
        { filter: batch.map(g => `Ref_Key eq guid'${g}'`).join(" or "), select: "Ref_Key,Description", top: BATCH },
      ).catch(() => [] as { Ref_Key: string; Description: string }[]);
      for (const item of items) nameMap.set(item.Ref_Key, item.Description);
    }
  ...
  async getPayrollDocuments(
    dateFrom: string,
    dateTo: string,
    organizationGuid?: string,
  ): Promise<{
    docs: {
      docGuid: string; date: string; number: string; period: string;
      totalAmount: number; employeeCount: number;
      lines: { employeeName: string; accrualType: string; amount: number }[];
    }[];
    totals: { totalAmount: number; docs: number };
  }> {
    const filters = [
      "Posted eq true", "DeletionMark eq false",
      `Date ge datetime'${dateFrom}T00:00:00'`,
    ...
    const payrollDocs = await this.client.getCollection<{
      Ref_Key: string; Date: string; Number: string; СуммаДокумента?: number; Период?: string;
    ...
    if (!payrollDocs.length) return { docs: [], totals: { totalAmount: 0, docs: 0 } };

    const docGuids = payrollDocs.map(d => d.Ref_Key);

    type AccrualLine = { Ref_Key: string; ФизЛицо_Key?: string; ВидНачисления?: string; Результат?: number };
    const BATCH = 10;
    const allLines: AccrualLine[] = [];
    for (let i = 0; i < docGuids.length; i += BATCH) {
      const batch = docGuids.slice(i, i + BATCH);
      const r = await this.client.getCollection<AccrualLine>(
        "Document_НачислениеЗарплатыРаботникамОрганизаций_Начисления",
        { filter: batch.map(g => `Ref_Key eq guid'${g}'`).join(" or "), select: "Ref_Key,ФизЛицо_Key,ВидНачисления,Результат", top: BATCH * 100 },
      ).catch(() => []);
      allLines.push(...r);
    }

    const empGuids = [...new Set(allLines.map(l => l.ФизЛицо_Key).filter(Boolean) as string[])];
    ...
    for (let i = 0; i < empGuids.length; i += BATCH) {
      ...
      for (const r of rows) empNames.set(r.Ref_Key, r.Description);
    }

    const linesByDoc = new Map<string, AccrualLine[]>();
    for (const line of allLines) {
      if (!linesByDoc.has(line.Ref_Key)) linesByDoc.set(line.Ref_Key, []);
      linesByDoc.get(line.Ref_Key)!.push(line);
    }
  ...
  async getStockReport(
    ...
    asOfDate: string;
    procurementWindow: { from: string; to: string };
    rows: {
      ...
      warehouses: { warehouseGuid: string; warehouseName: string; quantity: number; costValue: number }[];
    }[];
    totals: { totalCostValue: number; itemCount: number; zeroStockItems: number };
  }> {
    ...
    const stockAccounts = await this.client.getCollection<{ Ref_Key: string; Code: string }>(
      "ChartOfAccounts_Типовой",
      { filter: "Code eq '1310' or Code eq '1320' or Code eq '1330'", select: "Ref_Key,Code", top: 10 },
    );
    const defaultFromDate = new Date(asOf); defaultFromDate.setFullYear(defaultFromDate.getFullYear() - 1);
    const resolvedDateFrom = dateFrom ?? defaultFromDate.toISOString().slice(0, 10);
    if (!stockAccounts.length) return { asOfDate: asOf, procurementWindow: { from: resolvedDateFrom, to: asOf }, rows: [], totals: { totalCostValue: 0, itemCount: 0, zeroStockItems: 0 } };
    ...
    const filters: string[] = [`(${accFilter})`];
    if (organizationGuid) filters.push(`Организация_Key eq guid'${organizationGuid}'`);

    const balanceRows = await this.client.getRegisterBalance<{
      Account_Key?:        string;
      ExtDimension1?:      string;
    ...
    // Group by nomenclature (ExtDimension1), accumulate by warehouse (ExtDimension2)
    type NomAgg = {
      totalQty:   number;
      totalCost:  number;
      accountCode: string;
      warehouses: Map<string, { qty: number; cost: number }>;
    };
    const byNom = new Map<string, NomAgg>();
    for (const r of balanceRows) {
      // Only include rows where ExtDimension1 is Nomenclature
      if (!r.ExtDimension1_Type?.includes("Catalog_Номенклатура")) continue;
      const nomKey = r.ExtDimension1 ?? "";
      if (!nomKey) continue;
      if (warehouseGuid && r.ExtDimension2 !== warehouseGuid) continue;
      ...
      agg.totalQty  += qty;
      agg.totalCost += cost;
      if (!agg.accountCode) agg.accountCode = accCodeMap.get(r.Account_Key ?? "") ?? "";
    ...
    const nomDetails = new Map<string, { name: string; article: string; unitKey: string }>();
    for (let i = 0; i < nomGuids.length; i += BATCH) {
      ...
      const rows   = await this.client.getCollection<{
        Ref_Key: string; Description: string; Артикул?: string; БазоваяЕдиницаИзмерения_Key?: string;
    ...
    for (let i = 0; i < unitGuids.length; i += BATCH) {
      ...
      const rows  = await this.client.getCollection<{ Ref_Key: string; Description: string }>(
        ...
        { filter: chunk.map(g => `Ref_Key eq guid'${g}'`).join(" or "), select: "Ref_Key,Description", top: BATCH },
      ).catch(() => []);
      for (const r of rows) unitNames.set(r.Ref_Key, r.Description);
    }

    // Resolve warehouse names
    const whGuids = [...new Set(balanceRows.map(r => r.ExtDimension2).filter(Boolean) as string[])];
    ...
    for (let i = 0; i < whGuids.length; i += BATCH) {
      ...
      for (const r of rows) whNames.set(r.Ref_Key, r.Description);
    }
    ...
    if (organizationGuid) purchaseFilters.push(`Организация_Key eq guid'${organizationGuid}'`);

    const purchaseDocs = await this.client.getCollection<{
      Ref_Key: string; Date: string; Number: string; Контрагент_Key?: string;
    }>("Document_ПоступлениеТоваровУслуг", {
    ...
    const supplierGuids = [...new Set(purchaseDocs.map(d => d.Контрагент_Key).filter(Boolean) as string[])];
    ...
    for (let i = 0; i < supplierGuids.length; i += BATCH) {
      ...
      for (const r of rows) supplierNames.set(r.Ref_Key, r.Description);
    }

    // Fetch all табличные части in batches
    type PurchaseLine = { Ref_Key: string; Номенклатура_Key?: string; Цена?: number; Количество?: number };
    const purchaseLines: PurchaseLine[] = [];
    const docGuids = purchaseDocs.map(d => d.Ref_Key);
    for (let i = 0; i < docGuids.length; i += BATCH) {
      const chunk = docGuids.slice(i, i + BATCH);
      const rows  = await this.client.getCollection<PurchaseLine>(
    ...
    // Build: nomenclatureGuid → most recent purchase entry
    type LastPurchase = { price: number; date: string; docNumber: string; supplierGuid: string; supplierName: string };
    const lastPurchase = new Map<string, LastPurchase>();
    for (const line of purchaseLines) {
      const nomKey = line.Номенклатура_Key ?? "";
      if (!nomKey || !byNom.has(nomKey)) continue;
  ...
  async getSettlementBreakdown(
    ...
    organizationGuid?: string,
    date?: string,
  ): Promise<{ rows: SettlementRow[]; byAccount: Record<string, number> }> {
