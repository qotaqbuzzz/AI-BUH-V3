...
export class AnomalyMLService {
    ...
    // ── Public: full scan ─────────────────────────────────────────
    async scan(orgGuid, dateFrom, dateTo, baselines) {
        const t0 = Date.now();
        const anomalies = [];
    ...
    detectOutliers(docs, baselines) {
        const result = [];
        const baselineMap = new Map(baselines.map(b => [`${b.docType}::${b.month}`, b]));
        for (const doc of docs) {
            ...
            if (!bl || bl.stdDev === 0 || bl.sampleCount < 10)
                continue;
            const amount = doc.СуммаДокумента ?? 0;
            const z = (amount - bl.mean) / bl.stdDev;
            if (Math.abs(z) < Z_WARN)
                continue;
            const severity = Math.abs(z) >= Z_ERROR ? "error" : "warn";
    ...
    async buildBaselines(orgGuid, dateFrom, // pull 12+ months of history
    dateTo) {
        ...
        const addDoc = (doc, type) => {
            if (!doc.Date || !doc.СуммаДокумента)
                return;
    ...
    // ── Data fetchers ─────────────────────────────────────────────
    async fetchManualEntries(orgGuid, dateFrom, dateTo) {
        return this.client.getCollection("Document_ОперацияБух", {
            filter: [
    ...
    async fetchDocuments(entitySet, orgGuid, dateFrom, dateTo) {
        return this.client.getCollection(entitySet, {
            filter: [
                `DeletionMark eq false`,
                `Date ge datetime'${dateFrom}T00:00:00'`,
                `Date le datetime'${dateTo}T23:59:59'`,
...
//# sourceMappingURL=AnomalyMLService.js.map