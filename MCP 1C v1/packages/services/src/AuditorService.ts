...
export class AuditorService {
  ...
  async getDocumentJournalEntries(documentGuid: string): Promise<JournalEntry[]> {
    return this.client.getCollection<JournalEntry>("AccountingRegister_Типовой_RecordType", {
  ...
  async getESFStatus(organizationGuid: string, dateFrom: string, dateTo: string): Promise<unknown[]> {
  ...
  async getUnpostedDocuments(documentType: string, dateFrom: string, dateTo: string, organizationGuid?: string): Promise<unknown[]> {
    const filters = [
      `Posted eq false`,
  ...
  async auditPeriodQuality(
    ...
    const chk = (block: number, blockName: string, status: AuditStatus, message: string, value?: number | string): AuditCheck =>
      ({ block, blockName, status, message, value });

    // Organization name
    const orgRows = await this.client.getCollection<{ Ref_Key: string; Description: string }>(
      "Catalog_Организации",
      { filter: `Ref_Key eq guid'${organizationGuid}'`, select: "Ref_Key,Description", top: 1 },
    ...
    const unpostedByType = await Promise.all(
      ["РеализацияТоваровУслуг", "ПоступлениеТоваровУслуг", "ПлатежноеПоручениеИсходящее", "ПлатежноеПоручениеВходящее"].map(t =>
        this.getUnpostedDocuments(t, dateFrom, lastDay, organizationGuid).then(r => ({ type: t, count: r.length })),
      ),
    );
    ...
    const salaryDocs = await this.client.getCollection(
    ...
    const manualEntries = await this.client.getCollection<{
    ...
    const nightEntries = manualEntries.filter(r => {
      try { const h = new Date(r.Date).getHours(); return h < 7 || h >= 19; } catch { return false; }
    });
    ...
    const roundBatches = await Promise.all(
      ["РеализацияТоваровУслуг", "ПоступлениеТоваровУслуг", "ОперацияБух"].map(dt =>
        this.client.getCollection<{ СуммаДокумента?: number }>(`Document_${dt}`, {
          filter: `Организация_Key eq guid'${organizationGuid}' and DeletionMark eq false and Date ge datetime'${dateFrom}T00:00:00' and Date le datetime'${lastDay}T23:59:59'`,
    ...
    if (stornoRows.length === 0) {
      ...
      const detail = stornoRows.slice(0, 4).map(r =>
        `${r.accountCode}(${r.turnoverDr < 0 ? "Дт " + fmtN(r.turnoverDr) : "Кт " + fmtN(r.turnoverCr)})`,
      ).join(", ");
    ...
    if (atypical.length === 0) {
      ...
      const statusVal: AuditStatus = atypical.length > 3 ? "error" : "warn";
    ...
    const seasonCheck = (() => {
      if (month <= 3) return bal8110 > 100_000
      ...
      if (month <= 9) return bal8110 > 5_000_000
    ...
    if (prevRev > 0) {
      ...
      const sign = chg >= 0 ? "+" : "";
      if (Math.abs(chg) > 0.5) {
        checks.push(chk(5, "ДИНАМИКА", "warn",
          `Выручка: ${fmtN(currRev)} тг vs ${fmtN(prevRev)} тг пред. период (${sign}${(chg * 100).toFixed(0)}%) — резкое изменение`, chg));
      } else {
        checks.push(chk(5, "ДИНАМИКА", "ok",
          `Выручка: ${fmtN(currRev)} тг (${sign}${(chg * 100).toFixed(0)}% к пред. периоду)`));
      }
    } else {
  ...
  async getMonthCloseStatus(organizationGuid: string, year: number, month: number): Promise<{
    found: boolean;
    posted: boolean;
    date: string | null;
    guid: string | null;
    ...
    const _d = new Date(year, month, 0);
    const monthEnd = `${_d.getFullYear()}-${String(_d.getMonth()+1).padStart(2,"0")}-${String(_d.getDate()).padStart(2,"0")}`;

    const rows = await this.client.getCollection<{ Ref_Key: string; Date: string; Posted: boolean }>(
    ...
    if (rows.length === 0) return { found: false, posted: false, date: null, guid: null };
