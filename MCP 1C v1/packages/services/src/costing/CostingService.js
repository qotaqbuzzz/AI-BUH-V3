...
export class CostingService {
    ...
    async getNomenclatureUnitCost(nomenclatureGuid, dateFrom, dateTo, organizationGuid) {
        ...
        const fetchTurnovers = async (accountGuid) => {
            if (!accountGuid)
                return [];
            const filters = [`Account_Key eq guid'${accountGuid}'`];
            if (organizationGuid)
                filters.push(`Организация_Key eq guid'${organizationGuid}'`);
            return this.client.getRegisterTurnovers("AccountingRegister_Типовой", {
        ...
        const fetchBalance = async (accountGuid, date) => {
            if (!accountGuid)
                return [];
            const filters = [`Account_Key eq guid'${accountGuid}'`];
            if (organizationGuid)
                filters.push(`Организация_Key eq guid'${organizationGuid}'`);
            return this.client.getRegisterBalance("AccountingRegister_Типовой", {
                filter: filters.join(" and "),
        ...
        const isTargetNom = (r) => r.ExtDimension1 === nomenclatureGuid && r.ExtDimension1_Type?.includes("Catalog_Номенклатура");
        const sum1320 = rows1320.filter(isTargetNom).reduce((a, r) => ({ dr: a.dr + (r.СуммаTurnoverDr ?? 0), cr: a.cr + (r.СуммаTurnoverCr ?? 0), qDr: a.qDr + (r.КоличествоTurnoverDr ?? 0), qCr: a.qCr + (r.КоличествоTurnoverCr ?? 0) }), { dr: 0, cr: 0, qDr: 0, qCr: 0 });
        const sum7010 = rows7010.filter(isTargetNom).reduce((a, r) => ({ dr: a.dr + (r.СуммаTurnoverDr ?? 0), qDr: a.qDr + (r.КоличествоTurnoverDr ?? 0) }), { dr: 0, qDr: 0 });
        const sumBal = (rows) => rows.filter(isTargetNom).reduce((a, r) => {
    ...
    async getCOGSCompositionWithDocs(dateFrom, dateTo, organizationGuid, perCategoryDocLimit = 10) {
        ...
        for (const r of allRows) {
            ...
            if (r.recorderKey)
                existing.recorders.add(r.recorderKey);
    ...
    async resolveCatalogNames(guids, catalog) {
        const map = new Map();
        const unique = [...new Set(guids.filter(Boolean))];
        for (let i = 0; i < unique.length; i += BATCH) {
            const batch = unique.slice(i, i + BATCH);
            const rows = await this.client.getCollection(catalog, { filter: batch.map(g => `Ref_Key eq guid'${g}'`).join(" or "), select: "Ref_Key,Description", top: BATCH }).catch(() => []);
            for (const r of rows)
                map.set(r.Ref_Key, r.Description ?? r.Ref_Key.slice(0, 8));
        }
        return map;
    }
    async getRealProductionCosts(dateFrom, dateTo, organizationGuid, productionAccountCode = "8112", wipAccountCode = "1341", finishedGoodsAccountCode = "1320") {
        ...
        if (!guidProd)
            throw new Error(`Account ${productionAccountCode} not found`);
        ...
        const byCostItem = [...costMap.entries()]
            .sort((a, b) => b[1] - a[1])
            .map(([guid, amt]) => ({
        ...
        const byProduct = [...harvestMap.entries()]
            .sort((a, b) => b[1] - a[1])
            .map(([guid, amt]) => ({
    ...
    async resolveNomenclatureDetails(guid) {
        ...
        if (!rows.length)
            return { name: guid.slice(0, 8), unit: "" };
        const row = rows[0];
        const unitKey = row.БазоваяЕдиницаИзмерения_Key ?? "";
        let unit = "";
        if (unitKey) {
            const uRows = await this.client.getCollection("Catalog_ЕдиницыИзмерения", { filter: `Ref_Key eq guid'${unitKey}'`, select: "Ref_Key,Description", top: 1 }).catch(() => []);
            unit = uRows[0]?.Description ?? "";
        }
        return { name: row.Description ?? guid.slice(0, 8), unit };
    }
    async resolveRecorderDocs(guids) {
        if (!guids.length)
            return [];
        const found = new Map();
        for (const docType of RECORDER_DOC_TYPES) {
            ...
            for (let i = 0; i < remaining.length; i += BATCH) {
                ...
                const rows = await this.client.getCollection(`Document_${docType}`, {
                    filter,
        ...
        const contractorGuids = [...new Set([...found.values()].map(d => d.contractor).filter(Boolean))];
        const contractorNames = new Map();
        for (let i = 0; i < contractorGuids.length; i += BATCH) {
            const batch = contractorGuids.slice(i, i + BATCH);
            const rows = await this.client.getCollection("Catalog_Контрагенты", { filter: batch.map(g => `Ref_Key eq guid'${g}'`).join(" or "), select: "Ref_Key,Description", top: BATCH }).catch(() => []);
            for (const r of rows)
                contractorNames.set(r.Ref_Key, r.Description ?? "");
        }
        return guids
