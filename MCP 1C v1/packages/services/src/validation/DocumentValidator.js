...
export class DocumentValidator {
    ...
    async validateLineTotals(documentType, dateFrom, dateTo, organizationGuid, sampleLimit = 50) {
        ...
        if (organizationGuid)
            headerFilters.push(`Организация_Key eq guid'${organizationGuid}'`);
        const headers = await this.client.getCollection(`Document_${documentType}`, { filter: headerFilters.join(" and "), select: "Ref_Key,Number,Date,СуммаДокумента,СуммаНДС", top: sampleLimit, orderby: "Date desc" }).catch(() => []);
        if (headers.length === 0) {
            add(report, {
                ruleId: "DOC-001",
        ...
        for (const hdr of headers) {
            const lines = await this.client.getCollection(`Document_${documentType}_Товары`, { filter: `Ref_Key eq guid'${hdr.Ref_Key}'`, select: "LineNumber,Количество,Цена,Сумма,СуммаНДС,Номенклатура_Key", top: 500 }).catch(() => []);
            if (lines.length === 0)
                continue;
            checked++;
            let lineSum = 0;
    ...
    async validateNomenclatureAccounts(date, organizationGuid) {
        const start = Date.now();
        ...
        if (nomGuids.length > 0) {
            ...
            const items = await this.client.getCollection("Catalog_Номенклатура", { filter, select: "Ref_Key,Description,Услуга", top: 50 }).catch(() => []);
    ...
    async validateAdvanceAging(date, organizationGuid, agingDays = 90) {
        const start = Date.now();
