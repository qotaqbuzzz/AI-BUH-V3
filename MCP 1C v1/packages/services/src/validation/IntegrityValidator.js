...
export class IntegrityValidator {
    ...
    reports;
    register;
    constructor(client, reports, register) {
        this.client = client;
        this.reports = reports;
        this.register = register;
    }
    async validateDoubleEntry(dateFrom, dateTo, organizationGuid, perDocumentSampleLimit = 0) {
        ...
        if (perDocumentSampleLimit > 0) {
            ...
            const journal = await this.client.getCollection("AccountingRegister_Типовой_RecordType", {
                filter: filters.join(" and "),
                select: "Регистратор,Период,Сумма",
                top: Math.min(perDocumentSampleLimit * 50, 5000),
            }).catch(() => []);
            const perRecorder = new Map();
            ...
            for (const [recorder, e] of perRecorder.entries()) {
                if (Math.abs(e.dr - e.cr) > 1) {
                    flagged++;
                    if (flagged > perDocumentSampleLimit)
                        break;
                    add(report, {
                        ruleId: "INT-001d",
    ...
    async validateAccountSigns(dateFrom, dateTo, organizationGuid) {
        const start = Date.now();
    ...
    async validateBalanceArithmetic(dateFrom, dateTo, organizationGuid) {
        const start = Date.now();
