...
export class PeriodCloseValidator {
    ...
    register;
    auditor;
    constructor(client, register, auditor) {
        this.client = client;
        this.register = register;
        this.auditor = auditor;
    }
    async validatePeriodCloseReadiness(year, month, organizationGuid) {
    ...
    async validateAccrualAlignment(dateFrom, dateTo, organizationGuid) {
        const start = Date.now();
    ...
    async validateDepreciationCompleteness(dateFrom, dateTo, organizationGuid) {
        const start = Date.now();
    ...
    async validateWIPClosure(year, month, organizationGuid) {
        const start = Date.now();
        const pad = (n) => String(n).padStart(2, "0");
        const dateFrom = `${year}-${pad(month)}-01`;
        const lastDay = `${year}-${pad(month)}-${pad(new Date(year, month, 0).getDate())}`;
        ...
        const balances = await Promise.all(accounts.map(c => this.register.getAccountBalance(c, organizationGuid, lastDay).catch(() => null)));
    ...
    async validateCOGSBasis(dateFrom, dateTo, organizationGuid) {
        const start = Date.now();
...
//# sourceMappingURL=PeriodCloseValidator.js.map