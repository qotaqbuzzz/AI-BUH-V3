...
export class TaxValidator {
    ...
    async validateVATCharged(dateFrom, dateTo, organizationGuid) {
    ...
    async validateVATRecoverable(dateFrom, dateTo, organizationGuid) {
        const start = Date.now();
    ...
    async validateESFCoverage(dateFrom, dateTo, organizationGuid) {
        const start = Date.now();
        ...
        const sales = await this.client.getCollection("Document_РеализацияТоваровУслуг", { filter: filters.join(" and "), select: "Ref_Key,Number,Date,СуммаДокумента", top: 2000 }).catch(() => []);
        if (sales.length === 0) {
            add(report, {
                ruleId: "TAX-003",
        ...
        if (organizationGuid)
            esfFilters.push(`Организация_Key eq guid'${organizationGuid}'`);
        const esfRows = await this.client.getCollection("InformationRegister_АктуальныеЭСФ", { filter: esfFilters.join(" and "), select: "ДокументОснование,ДокументОснование_Key", top: 2000 }).catch(() => []);
        const esfRefs = new Set();
    ...
    async validatePayrollTaxRates(dateFrom, dateTo, organizationGuid) {
        const start = Date.now();
        ...
        const expect = (label, code, actual, expected, tolerancePct = 2) => {
    ...
    async validatePayrollDeductions(dateFrom, dateTo, organizationGuid) {
        const start = Date.now();
        ...
        if (organizationGuid)
            filters.push(`Организация_Key eq guid'${organizationGuid}'`);
        const docs = await this.client.getCollection("Document_НачислениеЗарплатыРаботникамОрганизаций", { filter: filters.join(" and "), select: "Ref_Key,Number,Date,СуммаДокумента", top: 500 }).catch(() => []);
        if (docs.length === 0) {
            add(report, {
                ruleId: "TAX-005",
    ...
    async validatePayrollAccrualBalance(dateFrom, dateTo, organizationGuid) {
        const start = Date.now();
        ...
        const [docs, t3350] = await Promise.all([
            this.client.getCollection("Document_НачислениеЗарплатыРаботникамОрганизаций", { filter: filters.join(" and "), select: "Ref_Key,СуммаДокумента", top: 1000 }).catch(() => []),
            this.register.getAccountTurnovers("3350", dateFrom, dateTo, organizationGuid).catch(() => null),
        ]);
...
//# sourceMappingURL=TaxValidator.js.map