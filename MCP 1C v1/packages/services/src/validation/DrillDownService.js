...
export class DrillDownService {
    ...
    async drillAccountSignViolation(accountCode, dateFrom, dateTo, organizationGuid, limit = 20) {
        ...
        for (const row of card.rows) {
            const e = byRecorder.get(row.recorderKey) ?? { dr: 0, cr: 0, period: row.period, corrAccounts: new Set() };
            e.dr += row.amountDr;
            e.cr += row.amountCr;
            if (row.corrAccountCode)
                e.corrAccounts.add(row.corrAccountCode);
            if (!e.period || row.period < e.period)
                e.period = row.period;
    ...
    async drillPayrollTaxDeviation(taxAccountCode, dateFrom, dateTo, organizationGuid) {
        const start = Date.now();
        ...
        if (organizationGuid)
            docFilters.push(`Организация_Key eq guid'${organizationGuid}'`);
        const payrollDocs = await this.client.getCollection("Document_НачислениеЗарплатыРаботникамОрганизаций", { filter: docFilters.join(" and "), select: "Ref_Key,Number,Date,СуммаДокумента", top: 500 }).catch(() => []);
    ...
    async drillMissingESF(dateFrom, dateTo, organizationGuid) {
        const start = Date.now();
        ...
        if (organizationGuid)
            saleFilters.push(`Организация_Key eq guid'${organizationGuid}'`);
        const [sales, esfRows] = await Promise.all([
            this.client.getCollection("Document_РеализацияТоваровУслуг", { filter: saleFilters.join(" and "), select: "Ref_Key,Number,Date,СуммаДокумента,СуммаНДС", top: 2000 }).catch(() => []),
            this.client.getCollection("InformationRegister_АктуальныеЭСФ", {
                filter: [
        ...
        const esfSet = new Set(esfRows.map(e => e.ДокументОснование_Key ?? e.ДокументОснование).filter(Boolean));
        let covered = 0;
        let idx = 0;
        for (const sale of sales) {
            idx++;
            if (esfSet.has(sale.Ref_Key)) {
                covered++;
                continue;
            }
    ...
    async drillStaleAdvances(date, accountCode, organizationGuid, agingDays = 90) {
        const start = Date.now();
        ...
        const contractStartMap = new Map();
        for (let i = 0; i < contractGuids.length; i += 20) {
            const batch = contractGuids.slice(i, i + 20);
            const rows = await this.client.getCollection("Catalog_ДоговорыКонтрагентов", { filter: batch.map(g => `Ref_Key eq guid'${g}'`).join(" or "), select: "Ref_Key,СрокДействияС", top: batch.length + 5 }).catch(() => []);
            for (const r of rows)
                if (r.СрокДействияС)
                    contractStartMap.set(r.Ref_Key, r.СрокДействияС);
        }
    ...
    async drillVATDocuments(dateFrom, dateTo, organizationGuid) {
        const start = Date.now();
        ...
        if (organizationGuid)
            saleFilters.push(`Организация_Key eq guid'${organizationGuid}'`);
        const sales = await this.client.getCollection("Document_РеализацияТоваровУслуг", { filter: saleFilters.join(" and "), select: "Ref_Key,Number,Date,СуммаДокумента,СуммаНДС", top: 2000 }).catch(() => []);
        if (sales.length === 0) {
    ...
    async drillWIPSourceDocuments(dateFrom, dateTo, organizationGuid) {
        const start = Date.now();
        ...
        for (const row of card.rows) {
            if (row.amountDr > 0) {
                inputs.set(row.corrAccountCode, (inputs.get(row.corrAccountCode) ?? 0) + row.amountDr);
                totalIn += row.amountDr;
            }
            if (row.amountCr > 0) {
                outputs.set(row.corrAccountCode, (outputs.get(row.corrAccountCode) ?? 0) + row.amountCr);
                totalOut += row.amountCr;
            }
        }
        add(report, {
    ...
    async drillUnpostedDocuments(documentType, dateFrom, dateTo, organizationGuid) {
        const start = Date.now();
        ...
        const docs = await this.client.getCollection(`Document_${documentType}`, { filter: filters.join(" and "), select: "Ref_Key,Number,Date,СуммаДокумента", orderby: "Date asc", top: 500 }).catch(() => []);
        if (docs.length === 0) {
    ...
    async drillUnpaidPayments(dateFrom, dateTo, organizationGuid) {
        const start = Date.now();
        ...
        if (organizationGuid)
            filters.push(`Организация_Key eq guid'${organizationGuid}'`);
        const docs = await this.client.getCollection("Document_ПлатежноеПоручениеИсходящее", { filter: filters.join(" and "), select: "Ref_Key,Number,Date,СуммаДокумента,Контрагент_Key", orderby: "Date asc", top: 200 }).catch(() => []);
        if (docs.length === 0) {
        ...
        const guids = [...new Set(docs.map(d => d.Контрагент_Key).filter(Boolean))];
        const contMap = new Map();
        for (let i = 0; i < guids.length; i += 20) {
            const batch = guids.slice(i, i + 20);
            const rows = await this.client.getCollection("Catalog_Контрагенты", { filter: batch.map(g => `Ref_Key eq guid'${g}'`).join(" or "), select: "Ref_Key,Description", top: batch.length }).catch(() => []);
            for (const r of rows)
                contMap.set(r.Ref_Key, r.Description);
        }
...
//# sourceMappingURL=DrillDownService.js.map