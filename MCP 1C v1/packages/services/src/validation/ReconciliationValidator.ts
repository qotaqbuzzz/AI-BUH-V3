...
export class ReconciliationValidator {
  ...
  async validateInvoicePaymentMatching(date: string, organizationGuid?: string, agingDaysWarn = 60): Promise<ValidationReport> {
  ...
  async validateContractTermsCompliance(dateFrom: string, dateTo: string, organizationGuid?: string): Promise<ValidationReport> {
    const start = Date.now();
    ...
    const contracts = await this.client.getCollection<{ Ref_Key: string; Description: string; СрокДействияС?: string; СрокДействияПо?: string }>(
      "Catalog_ДоговорыКонтрагентов",
      { filter: "DeletionMark eq false", select: "Ref_Key,Description,СрокДействияС,СрокДействияПо", top: 500 },
    ).catch(() => []);
    ...
    const expired = contracts.filter(c => {
      if (!c.СрокДействияПо) return false;
      try { return new Date(c.СрокДействияПо) < periodEnd; } catch { return false; }
    });
  ...
  async validateBankBalanceConsistency(dateFrom: string, dateTo: string, organizationGuid?: string): Promise<ValidationReport> {
    const start = Date.now();
    ...
    if (organizationGuid) payFilters.push(`Организация_Key eq guid'${organizationGuid}'`);

    const unpaid = await this.client.getCollection<{ Ref_Key: string; Number: string; Date: string; СуммаДокумента: number }>(
      "Document_ПлатежноеПоручениеИсходящее",
      { filter: payFilters.join(" and "), select: "Ref_Key,Number,Date,СуммаДокумента", top: 100 },
    ).catch(() => []);
